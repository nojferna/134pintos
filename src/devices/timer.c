...
timer_sleep (int64_t ticks) 
{
  ...
  list_insert_ordered(&blocked_list, &curr->sleep_elem, cmp_wake, NULL);
  thread_block();
  intr_set_level(old_level);
}

/* compare function to use for insert ordered based on wake time */
bool cmp_wake(const struct list_elem *a, const struct list_elem *b, void *aux) {
...
/* go throught list and wake threads who are ready */
static void
